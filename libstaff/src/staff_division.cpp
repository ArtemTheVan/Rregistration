...
QList<StaffDivision> StaffDivision::divisions() const
{
...
void StaffDivision::setDivisions(const QList<StaffDivision> &divisions)
{
    CHECK_D(d_ptr);
    Q_D(StaffDivision);
...
bool StaffDivision::addObject(StaffObject &object)
{
    ...
    if( object.isDivision() ) {
        StaffDivision obj = *static_cast<StaffDivision*>(&object);
        if( d->divisions.contains(obj) ) return(false);
        return( addDivision(obj) );

    } else if( object.isSoldier() ) {
        StaffSoldier obj = *static_cast<StaffSoldier*>(&object);
        if( d->soldiers.contains(obj) ) return(false);
        return( addSoldier(obj) );

    } else if( object.isCommander() ) {
        StaffCommander obj = *static_cast<StaffCommander*>(&object);
//        if( d->commander == (object.toCommander()) ) return(false); //todo uncomment if it need
...
bool StaffDivision::removeSoldier(StaffSoldier &soldier)
{
    ...
    if( d->soldiers.contains(soldier) ) {
        soldier.d_func()->setCommander(nullptr);
...
bool StaffDivision::removeSoldier(const QString &name)
{
    ...
    while (it != d->soldiers.end()) {
        if( it->name() == name )
        {
            (*it).d_func()->setCommander(nullptr);
            d->soldiers.erase(it);
            return(true);
        }
        ++it;
...
void StaffDivision::initEquipment()
{
    ...
    initEquipment(*((StaffObject*)this));
}
...
void StaffDivision::initEquipment(StaffObject obj)
{
    ...
    if(obj.isDivision()){
        StaffObjectList obj_lst = static_cast<StaffDivision *>(&obj)->objects();
        for(int i = 0; i < obj_lst.size(); ++i){
            StaffObject subobj = obj_lst[i];

            if(subobj.isBaseUnit()){
                d->equip += static_cast<StaffBaseUnit *>(&subobj)->equipment();
            }else if(subobj.isDivision()){
                static_cast<StaffDivision *>(&subobj)->initEquipment(*(static_cast<StaffDivision *>(&subobj)));

                d->equip += ((static_cast<StaffDivision *>(&subobj)))->equipmentConst();
            }
        }
    }
}
...
bool StaffDivision::contains(const StaffObject &object)
{
    ...
    if( object.isDivision() ) return( d->divisions.contains(*((StaffDivision*)&object)) );
    if( object.isSoldier() ) return( d->soldiers.contains(*((StaffSoldier*)&object)) );
    if( object.isCommander() ) return( d->commander == (*((StaffCommander*)&object)) );
