    ...
    StaffObjectPrivate(other, deep)
{
    ...
    if( copyLinks ) {
        ...
        // При глубоком клонировании сбрасываются все ссылки на родительские и соседние элементы.
...
void StaffBaseUnitPrivate::setParentDivision(StaffObjectPrivate *division) //todo it is StaffDivisionPrivate
{
    if( !division || division == m_parentDivision ) return;
    m_parentDivision = division;

    m_nameDivision = static_cast<StaffDivisionPrivate*>(m_parentDivision)->name; //todo it is not safe
    nameFullPart = static_cast<StaffDivisionPrivate*>(m_parentDivision)->nameFullPart; //todo it is not safe
...
void StaffBaseUnitPrivate::setRegistrationState(bool registered)
{
    ...
    {
        if( m_registered ) {
            if( !((ESUStaffEngine*)engine)->nodesRegistered.contains(nameFull) )
                ((ESUStaffEngine*)engine)->nodesRegistered.insert(nameFull, this);
        } else {
            if( ((ESUStaffEngine*)engine)->nodesRegistered.contains(nameFull) )
                ((ESUStaffEngine*)engine)->nodesRegistered.remove(nameFull);
        }
    }
}
...
void StaffBaseUnitPrivate::setNetworkAddress(const QHostAddress &address)
{
    ...
    QList<StaffObjectPrivate*> nodes = ((ESUStaffEngine*)engine)->nodesNetAddress.values();
    bool containsNode = nodes.contains((StaffObjectPrivate*)this);

    if( nodes.isEmpty() && !m_netAddress.isNull() ) {
        ((ESUStaffEngine*)engine)->nodesNetAddress.insert(m_netAddress, (StaffObjectPrivate*)this);
        return;
    } else if(  nodes.isEmpty() && m_netAddress.isNull() ) {
        return;
    }

    if( !m_netAddress.isNull() && !((ESUStaffEngine*)engine)->nodesNetAddress.contains(m_netAddress) )
    {
        if( containsNode ) {
            // Изменение NET адреса
            for( QHash<QHostAddress, StaffObjectPrivate*>::iterator it = ((ESUStaffEngine*)engine)->nodesNetAddress.begin(); it != ((ESUStaffEngine*)engine)->nodesNetAddress.end(); ++it )
            {
                if( this == it.value() ) {
                    it = ((ESUStaffEngine*)engine)->nodesNetAddress.erase(it);
                    ((ESUStaffEngine*)engine)->nodesNetAddress.insert(m_netAddress, (StaffObjectPrivate*)this);
                    break;
                }
            }
