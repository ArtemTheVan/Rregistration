...
Window {
    ...
    onWindowClosed: windowManager.showWindow("StructureForm");
    ...
    BusyBoxIndicator {
        ...
        onCloseEvent: {
            if( busyIndicator.canceledState ) {
                regAppUI.currentMessage = "Отмена регистрации";
                regApp.state = ESURegistration.RegistrationCanceled;
                closeTimer.start();
            } else {
                if(busyIndicator.closeMainWindow) closeTimer.start();
            }
        }
    }
